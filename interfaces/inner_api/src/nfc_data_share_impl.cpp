...
std::shared_ptr<DataShare::DataShareHelper> NfcDataShareImpl::CreateDataShareHelper()
{
    ...
    return DataShare::DataShareHelper::Creator(remoteObj_, NFC_DATA_URI);
}
...
KITS::ErrorCode NfcDataShareImpl::GetValue(Uri &uri, const std::string &column, int32_t &value)
{
    ...
    predicates.EqualTo(NFC_DATA_COLUMN_KEYWORD, column);
    ...
    rows->GoToFirstRow();
    int32_t columnIndex;
    rows->GetColumnIndex(NFC_DATA_COLUMN_VALUE, columnIndex);
    std::string valueStr;
...
KITS::ErrorCode NfcDataShareImpl::SetValue(Uri &uri, const std::string &column, int &value)
{
    ...
    bucket.Put(NFC_DATA_COLUMN_VALUE, valueObj);
    bucket.Put(NFC_DATA_COLUMN_KEYWORD, keyObj);
    int32_t err;
