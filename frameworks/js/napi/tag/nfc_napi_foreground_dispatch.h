...
public:
    ...
    int Register(const napi_env &env, ElementName &element, std::vector<uint32_t> &discTech, napi_value handler);
    int Unregister(const napi_env &env, ElementName &element, napi_value handler);

private:
    int RegisterForegroundEvents(ElementName &element, std::vector<uint32_t> &discTech);
...
public:
    ...
    int RegisterWithIntvl(const napi_env &env, ElementName &element,
...
private:
    ...
    int RegReaderModeEvtWithIntvl(ElementName &element, std::vector<uint32_t> &discTech, int interval);
